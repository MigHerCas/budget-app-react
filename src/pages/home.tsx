...
import BudgetItemAdd from './../components/budgetItemAdd';
...
// HomePage component
const HomePage = (props: HomePageInterface): JSX.Element => {
  ...
  function handleItemUpdate(value: string, id: string, itemProperty: string) {
    ...
    switch (itemProperty) {
      case 'isPaid':
        ...
        newBudgetItemsState.find((item: BudgetItemObjInterface) => item.id === id)!.isPaid = !newBudgetItemsState.find(
          (item: BudgetItemObjInterface) => item.id === id,
        )!.isPaid;
        break;
      case 'price':
  ...
  function handleItemRemove(id: string) {
    ...
    const newBudgetItemsState = props.budgetItems.filter((item: BudgetItemObjInterface) => item.id !== id);
  ...
  return (
    <div>
      ...
      {showAddItem && (
        <BudgetItemAdd showAddItem={showAddItem} handleShowAddItem={setShowAddItem} handleAddItem={handleAddItem} />
      )}

      <button className="btn btn-add" onClick={() => setShowAddItem(!showAddItem)}>
